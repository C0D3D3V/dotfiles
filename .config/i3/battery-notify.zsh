...
DBUS="qdbus"
ICON="/home/daniel/.config/i3/low-battery.png"
INTERVAL=60
REPEAT_WARNING=true
...
VERBOSE=false
STATUS="OK"
SOUND_FILE="$HOME/.config/i3/i3-battery-popup.wav"
VOLUME=60
...
get_onbattery() {
    if [[ $DBUS = "qdbus" ]]; then
        onbattery=$(qdbus --system org.freedesktop.UPower \
            /org/freedesktop/UPower \
           org.freedesktop.UPower.OnBattery)
    elif [[ $DBUS = "gdbus" ]]; then
        onbattery=$(gdbus call --system --dest org.freedesktop.UPower \
...
loop() {
    ...
    get_onbattery
    if [[ $onbattery = "true" ]]; then
        notify
    else
        [[ $VERBOSE = true ]] && echo "Skip notify check, because the system is not running on battery"
    fi
    sleep "$INTERVAL"
}
