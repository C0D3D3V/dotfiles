...
# copys the main identity to server (~/.ssh/main.pub) and removes old ssh keys from server (see next function)
ssh-update-key(){
...
pdf() {
	RG_PREFIX="rga --files-with-matches -i -t pdf"
	local file
  file="$(
    FZF_DEFAULT_COMMAND="$RG_PREFIX '$*'" \
      fzf --sort --preview="[[ ! -z {} ]] && rga --pretty -i --context 20 {q} {}" \
        --phony -q "$*" \
        --bind "change:reload:$RG_PREFIX {q}" \
        --preview-window="80%:down" \
        --bind 'ctrl-y:execute-silent(nohup xdg-open {} &)'
  )" &&
  echo "opening $file" &&
  sh -c 'nohup xdg-open "$0" >/dev/null 2>&1 &' "$file"
}
...
ppdf() {
    ...
    | fzf --read0 --reverse -e -d $'\t'  \
        --preview-window down:80% --with-nth 1 --no-multi-line --preview '
            v=$(echo {q} | tr " " "|");
            echo -e {1}"\n"{2} | grep -E "^|$v" -i --color=always;
        ' \
    | cut -z -f 1 -d $'\t' | tr -d '\n' | xargs -r --null sh -c 'nohup "$0" "$@" >/dev/null 2>&1 &' "$open"
}
